...
class VideoPathDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        print(torch.cat(out_images, 0).shape)
        return {
...
def main(opt, model):
    ...
    for data in tqdm(dataloader):
        # TODO the original one would cause error?!
        # num_batch = data['imgs'][0].shape[0] // batch_size + 1
        # print("num_batch:",num_batch)
        num_batch = data['imgs'][0].shape[0] // batch_size
        pred_coeffs = list()
        for index in range(num_batch):
            # print('1 input img shape', data['imgs'].shape)
            # print("{},{}".format(index*batch_size,(index+1)*batch_size))
            data_input = {                
                'imgs': data['imgs'][0,index*batch_size:(index+1)*batch_size],
            }
            # print('2 input img shape', data_input['imgs'].shape)
        ...
        print('pred_trans_params:',pred_trans_params.shape)
        savemat(
