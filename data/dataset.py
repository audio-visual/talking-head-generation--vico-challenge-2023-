...
class InferenceDataset(data.Dataset):  # audio and first frames
    def __init__(self, dataset_dir, wav2lip, fps=30, sr=16000):
        ...
        if wav2lip:
            self.all_wav2lip_coeffs_fns = sorted(list(glob(f"{dataset_dir}/recons/arcface_wav2lip_41000/*speaker.mat")))
        else:
            self.all_wav2lip_coeffs_fns  = self.all_coeffs_fns.clone()
...
class TrainingDataset_pre(data.Dataset):
    ...
    def __getitem__(self, idx):
        batch_path = self.all_batch_fns[idx]
        # get the uuid
        uuid = batch_path.split(os.path.sep)[-2]
        # read the npy
        arcface_emb = np.load(os.path.join('/hy-tmp/train/first_frames/',uuid+'.npy'))
        return {'batch':torch.load(batch_path),'arcface':np.squeeze(arcface_emb)}
